...
# user to change
data_path=r"E:\Data"  # the r in front is convert a normal string to a raw string
...
def readRabiAPDFile(filename):
	datFrame= pd.read_csv(filename,header= None,skiprows=[0,1,2],sep= ' ',comment= 'X',names= ['time','signal','ref','ref2']).dropna(axis=0).astype(float)
	tauVals= datFrame['time'].values
	sigVals= datFrame['signal'].values
...
###################################################################### FUNCTIONS FOR WIDEFIELD DATA

def read_matfile(filename, normalize= True, avg= True, Nx= 4, Ny= 4, Nf= 4, norm_points= 10, normal= True):
    '''
    fVals, dat, xFrom, xTo, X, Y, npoints= read_matfile(filename, normalize= True, avg= True, Nx= 4, Ny= 4, Nf= 4, norm_points= 10, normal= True)
    '''
...
def plot_row(img,freqVals,row,width_ratios=[1,5],normalized= False):
    ...
    plot_row(img,freqVals,row,width_ratios=[1,5],normalized= False)
    '''
    if not(normalized):
...
def plot_col(img,freqVals,col,width_ratios= [1,4],normalized= False):
    ...
    plot_col(img,freqVals,col,width_ratios= [1,4],normalized= False)
    '''
    if not(normalized):
        img_n= normalize_widefield(img)
    else:
...
def generate_seed_parameters(dat, fVals, rowVals, colVals):
    '''
    seed_pOpts= generate_seed_parameters(dat, fVals, rowVals, colVals)
    '''
    X, Y, npoints= dat.shape
...
def getFitInfo(datfit, numpeaks, param):
    '''
    paramVals= getFitInfo(datfit, numpeaks, param)
    param can be 'frequency', 'linewidth', or 'contrast'

    Assumes that index 0 is background_value
    '''
    param= param.lower()
...
def interpolate(array):
    ...
    return griddata((x1, y1), newarr.ravel(),(xx, yy), method='cubic')
...
#### helper functions for fitting
def fit_function_BT(xVals,Bx,By,Bz, T= 300, init_group=1):
    '''
...
def computeFreqVals(Bx,By,Bz,T= 300, init_group= 1):
    ...
    freqVals= []
    for i in range(1,5):
        R= nv_transformation(i).dot(nv_transformation(init_group).transpose())
        H= NV_H(T,*R.dot(B))
...
def fit_function_BTP(xVals,Bx,By,Bz,T= 300,P= 0,init_group=1):
    '''
    ...
    freqVals= computeFreqVals_pressure(Bx,By,Bz,T,P,init_group)
    yFit= np.ones(xVals.shape)*background
    for a,x0,l in zip(ampVals,freqVals,FWHMVals):
        yFit+= gaussian_c(xVals,a,l,x0)
    return yFit

def computeFreqVals_pressure(Bx,By,Bz, T= 300, P= 0, init_group=1):
    '''
    freqVals= computeFreqVals(B,T)
